...
def train(model, model_six, optimizer, loss_fn, trainloader, valloader, device, maxi):
        max = maxi
        for epoch in range(10):
                ...
                for batch_num, (data, target) in enumerate(trainloader):
                        ...
                        if (target1 == 6).nonzero().shape[0] != 0:
                            indices = (target1 == 6).nonzero()[:,0]

                            six_data = next_data[indices]
                            six_target = target[indices]

                            six_target1 = six_target.clone().to(device)
                            six_target1[(six_target1 == 12).nonzero()] = 100
                            six_target1[(six_target1 == 39).nonzero()] = 101
                            six_target1 -= 100
                            loss = train_mod(model_six, optimizer, loss_fn, six_data, six_target1, device)
                            num_six += six_data.shape[0]

                            progress_bar(batch_num, len(trainloader), 'Loss: %.4f | Acc: %.3f%% (%d/%d)'
                             % (loss / (batch_num + 1), 100. * model_six.acc / num_six, model_six.acc, num_six))
                print("Epoch: ",epoch+1,"\nTrain: ", model_six.acc / num_six)
                ...
                for data, target in valloader:
                        ...
                        if (target1 == 6).nonzero().shape[0] != 0:
                            ...
                            _, six_out = model_six(six_data)
                            pred_six = six_out.max(1, keepdim=True)[1]
                            count_six += pred_six.eq(six_target1.view_as(pred_six)).sum().item()
                            num_six += six_data.shape[0]

                print("Val: ", count_six/num_six)
                if count_six/num_six > max:
                    print("checkpoint saved")
                    max = count_six/num_six
                    torch.save(model_six.state_dict(), "../Models/emnist_6.pth")
        return max
...
def main():
        ...
        #model_six.load_state_dict(torch.load('emnist_6.pth'))

        learning_rate = 0.001
        optimizer = torch.optim.Adam(model_six.parameters(), lr=learning_rate)
        loss_fn = nn.CrossEntropyLoss()
        max = train(model, model_six, optimizer, loss_fn, train_loader, val_loader, torch.device("cuda"), 0)
        for i in range(2):
            model_six.load_state_dict(torch.load('emnist_6.pth'))
            learning_rate /= 10
            optimizer = torch.optim.Adam(model_six.parameters(), lr=learning_rate)
            loss_fn = nn.CrossEntropyLoss()
            max = train(model, model_six, optimizer, loss_fn, train_loader, val_loader, torch.device("cuda"), max)
        model_six.load_state_dict(torch.load('emnist_6.pth'))
        #average_softmax(model, model_six, train_loader, val_loader, torch.device("cuda"))
