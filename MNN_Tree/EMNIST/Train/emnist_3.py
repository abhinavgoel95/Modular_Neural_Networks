...
def train(model, model_three, optimizer, loss_fn, trainloader, valloader, device, maxi):
        ...
        for epoch in range(10):
                ...
                for batch_num, (data, target) in enumerate(trainloader):
                        ...
                        three_target1[(three_target1 == 34).nonzero()] = 103
                        three_target1[(three_target1 == 10).nonzero()] = 104
                        three_target1[(three_target1 == 31).nonzero()] = 105
                        three_target1[(three_target1 == 33).nonzero()] = 106
                        three_target1[(three_target1 == 20).nonzero()] = 107
                        three_target1[(three_target1 == 27).nonzero()] = 108
                        three_target1[(three_target1 == 42).nonzero()] = 109
                        three_target1[(three_target1 == 37).nonzero()] = 110
                        three_target1[(three_target1 == 6).nonzero()] = 111
                        three_target1[(three_target1 == 16).nonzero()] = 112
                        three_target1 -= 100
                ...
                if count_three/num_three > max:
                    ...
                    torch.save(model_three.state_dict(), "../Models/emnist_3.pth")
        return max

def average_softmax(model, model_three, trainloader, valloader, device):
    nb_classes = 12
    out_classes = 12
    ...
    with torch.no_grad():
        for i, (data, target) in enumerate(trainloader):
            ...
            for categ in range(nb_classes):
                indices = (three_target1 == categ).nonzero()[:,0]
    ...
    for i in range(12):
        print(soft_out[i])
    torch.save(soft_out, 'emnist_3_softmax_output.pth')
...
def main():
        ...
        max = train(model, model_three, optimizer, loss_fn, train_loader, val_loader, torch.device("cuda"), .965)
        for i in range(2):
            model_three.load_state_dict(torch.load('emnist_3.pth'))
            learning_rate /= 10
        ...
        model_three.load_state_dict(torch.load('emnist_3.pth'))
        #average_softmax(model, model_three, train_loader, val_loader, torch.device("cuda"))
