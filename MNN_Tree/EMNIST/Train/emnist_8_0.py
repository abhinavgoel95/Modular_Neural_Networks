...
def train(model, model_eight, model_eight_zero, optimizer, loss_fn, trainloader, valloader, device, maxi):
        max = maxi
        for epoch in range(10):
                model.eval()
                model_eight.eval()
                model_eight_zero.train()
                count = 0
                num_eight_zero = 0
                total = 0
                model_eight_zero.acc = 0
                for batch_num, (data, target) in enumerate(trainloader):
                        ...
                        if (target1 == 8).nonzero().shape[0] != 0:
                            ...
                            next_data ,net_out = model_eight(eight_data)
                            if (eight_target1 == 0).nonzero().shape[0] != 0:
                                indices = (eight_target1 == 0).nonzero()[:,0]

                                eight_zero_data = next_data[indices]
                                eight_zero_target = eight_target[indices]

                                eight_zero_target1 = eight_zero_target.clone().to(device)
                                eight_zero_target1[(eight_zero_target1 == 15).nonzero()] = 100
                                eight_zero_target1[(eight_zero_target1 == 40).nonzero()] = 101
                                eight_zero_target1 -= 100

                                loss = train_mod(model_eight_zero, optimizer, loss_fn, eight_zero_data, eight_zero_target1, device)
                                num_eight_zero += eight_zero_data.shape[0]

                                progress_bar(batch_num, len(trainloader), 'Loss: %.4f | Acc: %.3f%% (%d/%d)'
                                 % (loss / (batch_num + 1), 100. * model_eight_zero.acc / num_eight_zero, model_eight_zero.acc, num_eight_zero))
                print("Epoch: ",epoch+1,"\nTrain: ", model_eight_zero.acc / num_eight_zero)
                ...
                for data, target in valloader:
                        ...
                        if (target1 == 8).nonzero().shape[0] != 0:
                            ...
                            if (eight_target1 == 0).nonzero().shape[0] != 0:
                                ...
                                _, eight_zero_out = model_eight_zero(eight_zero_data)
                                pred_eight_zero = eight_zero_out.max(1, keepdim=True)[1]
                                count_eight_zero += pred_eight_zero.eq(eight_zero_target1.view_as(pred_eight_zero)).sum().item()
                                num_eight_zero += eight_zero_data.shape[0]
                print("Val: ", count_eight_zero/num_eight_zero)
                if count_eight_zero/num_eight_zero > max:
                    print("checkpoint saved")
                    max = count_eight_zero/num_eight_zero
                    torch.save(model_eight_zero.state_dict(), "../Models/emnist_8_0.pth")
        return max
...
def main():
        ...
        model_eight_zero.load_state_dict(torch.load('emnist_8_0.pth'))

        learning_rate = 0.0001
        optimizer = torch.optim.Adam(model_eight_zero.parameters(), lr=learning_rate)
        loss_fn = nn.CrossEntropyLoss()
        max = train(model, model_eight, model_eight_zero, optimizer, loss_fn, train_loader, val_loader, torch.device("cuda"), 0)
        for i in range(2):
            model_eight_zero.load_state_dict(torch.load('emnist_8_0.pth'))
            learning_rate /= 10
            optimizer = torch.optim.Adam(model_eight_zero.parameters(), lr=learning_rate, weight_decay = 5e-4)
            loss_fn = nn.CrossEntropyLoss()
            max = train(model, model_eight, model_eight_zero, optimizer, loss_fn, train_loader, val_loader, torch.device("cuda"), max)
