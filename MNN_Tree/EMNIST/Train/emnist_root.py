...
def train(model, optimizer, loss_fn, trainloader, valloader, device, maxi):
        max = maxi
        for epoch in range(20):
                ...
                if count1/len(valloader.sampler.indices) > max:
                    ...
                    torch.save(model.state_dict(), "../Models/root_emnist.pth")
        return max
...
def main():
        ...
        max = train(model, optimizer, loss_fn, train_loader, val_loader, torch.device("cuda"), 0)
        for i in range(3):
            model.load_state_dict(torch.load('emnist_root.pth'))
            learning_rate /= 10
            optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate, weight_decay = 5e-4)
            loss_fn = nn.CrossEntropyLoss()
            max = train(model, optimizer, loss_fn, train_loader, val_loader, torch.device("cuda"), max)
