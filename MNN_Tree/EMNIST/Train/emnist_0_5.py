...
def train(model, model_zero, model_zero_five, optimizer, loss_fn, trainloader, valloader, device, maxi):
        max = maxi
        for epoch in range(10):
                model.eval()
                model_zero.eval()
                model_zero_five.train()
                count = 0
                num_zero_five = 0
                total = 0
                model_zero_five.acc = 0
                for batch_num, (data, target) in enumerate(trainloader):
                        ...
                        if (zero_target1 == 5).nonzero().shape[0] != 0:
                            indices = (zero_target1 == 5).nonzero()[:,0]

                            zero_five_data = next_data[indices]
                            zero_five_target = zero_target[indices]

                            zero_five_target1 = zero_five_target.clone().to(device)
                            zero_five_target1[(zero_five_target1 == 44).nonzero()] = 100
                            zero_five_target1[(zero_five_target1 == 41).nonzero()] = 101
                            zero_five_target1[(zero_five_target1 == 9).nonzero()] = 102
                            zero_five_target1 -= 100

                            loss = train_mod(model_zero_five, optimizer, loss_fn, zero_five_data, zero_five_target1, device)
                            num_zero_five += zero_five_data.shape[0]

                            progress_bar(batch_num, len(trainloader), 'Loss: %.4f | Acc: %.3f%% (%d/%d)'
                             % (loss / (batch_num + 1), 100. * model_zero_five.acc / num_zero_five, model_zero_five.acc, num_zero_five))
                print("Epoch: ",epoch+1,"\nTrain: ", model_zero_five.acc / num_zero_five)

                if model_zero_five.acc / num_zero_five > max:
                    print("checkpoint saved")
                    max = model_zero_five.acc / num_zero_five
                    torch.save(model_zero_five.state_dict(), "../Models/emnist_0_5.pth")
                ...
                for data, target in valloader:
                        ...
                        if (zero_target1 == 5).nonzero().shape[0] != 0:
                            ...
                            _, zero_five_out = model_zero_five(zero_five_data)
                            pred_zero_five = zero_five_out.max(1, keepdim=True)[1]
                            count_zero_five += pred_zero_five.eq(zero_five_target1.view_as(pred_zero_five)).sum().item()
                            num_zero_five += zero_five_data.shape[0]
                print("Val: ", count_zero_five/num_zero_five)
        return max
...
def main():
        ...
        optimizer = torch.optim.Adam(model_zero_five.parameters(), lr=learning_rate)
        loss_fn = nn.CrossEntropyLoss()
        max = train(model, model_zero, model_zero_five, optimizer, loss_fn, train_loader, val_loader, torch.device("cuda"), 0)
        for i in range(2):
            model_zero_five.load_state_dict(torch.load('emnist_0_5.pth'))
            learning_rate /= 10
            optimizer = torch.optim.Adam(model_zero_five.parameters(), lr=learning_rate, weight_decay = 5e-4)
            loss_fn = nn.CrossEntropyLoss()
            max = train(model, model_zero, model_zero_five, optimizer, loss_fn, train_loader, val_loader, torch.device("cuda"), max)
