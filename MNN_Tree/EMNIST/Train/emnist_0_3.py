...
def train(model, model_zero, model_zero_three, optimizer, loss_fn, trainloader, valloader, device, maxi):
        max = maxi
        for epoch in range(10):
                model.eval()
                model_zero.eval()
                model_zero_three.train()
                count = 0
                num_zero_three = 0
                total = 0
                model_zero_three.acc = 0
                for batch_num, (data, target) in enumerate(trainloader):
                        ...
                        if (zero_target1 == 3).nonzero().shape[0] != 0:
                            indices = (zero_target1 == 3).nonzero()[:,0]

                            zero_three_data = next_data[indices]
                            zero_three_target = zero_target[indices]

                            zero_three_target1 = zero_three_target.clone().to(device)
                            zero_three_target1[(zero_three_target1 == 2).nonzero()] = 100
                            zero_three_target1[(zero_three_target1 == 35).nonzero()] = 101
                            zero_three_target1 -= 100

                            loss = train_mod(model_zero_three, optimizer, loss_fn, zero_three_data, zero_three_target1, device)
                            num_zero_three += zero_three_data.shape[0]

                            progress_bar(batch_num, len(trainloader), 'Loss: %.4f | Acc: %.3f%% (%d/%d)'
                             % (loss / (batch_num + 1), 100. * model_zero_three.acc / num_zero_three, model_zero_three.acc, num_zero_three))
                print("Epoch: ",epoch+1,"\nTrain: ", model_zero_three.acc / num_zero_three)
                ...
                for data, target in valloader:
                        ...
                        if (zero_target1 == 3).nonzero().shape[0] != 0:
                            ...
                            _, zero_three_out = model_zero_three(zero_three_data)
                            pred_zero_three = zero_three_out.max(1, keepdim=True)[1]
                            count_zero_three += pred_zero_three.eq(zero_three_target1.view_as(pred_zero_three)).sum().item()
                            num_zero_three += zero_three_data.shape[0]
                print("Val: ", count_zero_three/num_zero_three)
                if count_zero_three/num_zero_three > max:
                    print("checkpoint saved")
                    max = count_zero_three/num_zero_three
                    torch.save(model_zero_three.state_dict(), "../Models/emnist_0_3.pth")
        return max
...
def main():
        ...
        optimizer = torch.optim.Adam(model_zero_three.parameters(), lr=learning_rate)
        loss_fn = nn.CrossEntropyLoss()
        max = train(model, model_zero, model_zero_three, optimizer, loss_fn, train_loader, val_loader, torch.device("cuda"), 0)
        for i in range(2):
            model_zero_three.load_state_dict(torch.load('emnist_0_3.pth'))
            learning_rate /= 10
            optimizer = torch.optim.Adam(model_zero_three.parameters(), lr=learning_rate, weight_decay = 5e-4)
            loss_fn = nn.CrossEntropyLoss()
            max = train(model, model_zero, model_zero_three, optimizer, loss_fn, train_loader, val_loader, torch.device("cuda"), max)
