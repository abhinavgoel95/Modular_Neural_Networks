...
def train(model, model_one, optimizer, loss_fn, trainloader, valloader, device, maxi):
        max = maxi
        for epoch in range(10):
                ...
                for batch_num, (data, target) in enumerate(trainloader):
                        ...
                        if (target1 == 1).nonzero().shape[0] != 0:
                            ...
                            one_target1 -= 100
                            loss = train_mod(model_one, optimizer, loss_fn, one_data, one_target1, device)
                            num_one += one_data.shape[0]

                            progress_bar(batch_num, len(trainloader), 'Loss: %.4f | Acc: %.3f%% (%d/%d)'
                             % (loss / (batch_num + 1), 100. * model_one.acc / num_one, model_one.acc, num_one))
                print("Epoch: ",epoch+1,"\nTrain: ", model_one.acc / num_one)
                ...
                for data, target in valloader:
                        ...
                        if (target1 == 1).nonzero().shape[0] != 0:
                            ...
                            _, one_out = model_one(one_data)
                            pred_one = one_out.max(1, keepdim=True)[1]
                            count_one += pred_one.eq(one_target1.view_as(pred_one)).sum().item()
                            num_one += one_data.shape[0]

                print("Val: ", count_one/num_one)
                if count_one/num_one > max:
                    print("checkpoint saved")
                    max = count_one/num_one
                    torch.save(model_one.state_dict(), "../Models/emnist_1.pth")
        return max

def average_softmax(model, model_one, trainloader, valloader, device):
    nb_classes = 4
    out_classes = 4
    ...
    with torch.no_grad():
        for i, (data, target) in enumerate(trainloader):
            ...
            for categ in range(nb_classes):
                indices = (one_target1 == categ).nonzero()[:,0]
    ...
    for i in range(4):
        print(soft_out[i])
    torch.save(soft_out, 'emnist_1_softmax_output.pth')
...
def main():
        ...
        optimizer = torch.optim.Adam(model_one.parameters(), lr=learning_rate)
        loss_fn = nn.CrossEntropyLoss()
        max = train(model, model_one, optimizer, loss_fn, train_loader, val_loader, torch.device("cuda"), 0)
        for i in range(2):
            model_one.load_state_dict(torch.load('emnist_1.pth'))
            learning_rate /= 10
            optimizer = torch.optim.Adam(model_one.parameters(), lr=learning_rate, weight_decay = 5e-4)
            loss_fn = nn.CrossEntropyLoss()
            max = train(model, model_one, optimizer, loss_fn, train_loader, val_loader, torch.device("cuda"), max)
        model_one.load_state_dict(torch.load('emnist_1.pth'))
        average_softmax(model, model_one, train_loader, val_loader, torch.device("cuda"))
